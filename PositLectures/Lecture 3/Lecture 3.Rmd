...
new_recorder <- function(tutorial_id, tutorial_version, user_id, event, data) {
  ...
  if (use_database && !is.null(conn)) {
    try(DBI::dbWriteTable(conn, "module3", d_tibble, append = TRUE, row.names = FALSE), silent = TRUE)
  }
}
...
output$Q1 <- renderPlot({
  ...
  data <- DBI::dbGetQuery(conn, "SELECT * FROM module3")
...
output$Q2 <- renderPlot({
  if (!(use_database && !is.null(conn))) return(NULL)
  data <- DBI::dbGetQuery(conn, "SELECT * FROM module12")
