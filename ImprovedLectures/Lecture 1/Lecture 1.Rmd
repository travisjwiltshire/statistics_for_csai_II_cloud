...
#Sys.setenv(LEARNR_USE_DB = "true") - uncomment to enable database features
...
new_recorder <- function(tutorial_id, tutorial_version, user_id, event, data) {
  ...
  ## Only prepare and write to the database when enabled and connected
  if (use_database && !is.null(conn)) {
    ...
    ## send to mysql
    DBI::dbWriteTable(conn, "module1", d_tibble, append = TRUE, row.names = FALSE)
  }
}
...
if (use_database && !is.null(conn)) {
  output$Q1 <- renderPlot({
    res <- DBI::dbSendQuery(conn, "SELECT * FROM module1")
