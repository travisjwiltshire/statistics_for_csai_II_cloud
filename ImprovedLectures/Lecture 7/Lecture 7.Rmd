...
## Toggle: enable/disable external logging (DB/Sheets)
...
## Ensure learnr exercise settings
...
## Introduction
  ...
  if (use_database && !is.null(conn)) {
    DBI::dbWriteTable(conn, "module7", g_tibble, append = TRUE, row.names = FALSE)
  }
    <div class='container'>
...
```
```{r ex11-hint}
albumsales2<-read.delim(file="Album Sales 2.dat",header=TRUE)
...
- Generate standardized beta coefficients for your multiple regression model using `lm.beta()` from QuantPsyc package
- Generate confidence intervals for the standardized estimates using `confint()`
...
### Generate confidence intervals for the standardized estimates 

- Use `lm.beta()` from QuantPsyc package
...
```{r ex43-hint}
gvlma(albumsales.5)
performance::check_model(albumsales.5)
```
```{r ex43-check}
#store
```
