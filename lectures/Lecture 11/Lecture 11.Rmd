...
## Multiple and Mixed Model Regression
...
### Interpreting Random Effects and Intraclass Correlation 
...
- Intraclass correlation
  ...
  - How much variation is explained due to clustering/grouping variable(s)
  - Compare how ICC changes with different changes to model

### Running the model in R with random intercepts


- Install the lme4 package and lmerTest package
...
### Load packages and load politness data


- Also check out the politeness data set (get a summary, make some plots, etc.)
- Make a prediction about the relationship between attitude and vocal frequency?
...
```
```{r ex11-hint}
politeness <- read.csv("~/Teaching/Statistics in R/Stats II CSAI Fall 2021/Data sets/politeness_data.csv")
summary(politeness)
...
coef(mod.2)
summary(mod.2)
# Marginal R-squared is the variance described by the fixed effects
# Conditional R-squared is the variance described by the fixed AND random effects together
```
```{r ex14-check}
#store
```
...
### Repeat the same mixed model with random intercepts and new predictor
...
- Run a mixed effects model to predict vocal frequency from attitude and gender while allowing random intercepts for subject and scenario
  - Remember to add these to the equation (1|pred)
- Get R2 values using performance() function
...
#What if we try a regular linear model?
mod.1<-lm(frequency~gender+attitude,data=politeness)
summary(mod.1)
...
```
```{r ex15-hint}
#Run our second mixed model with random intercepts for subject and scenario adding gender
...
### Exercise: Run a  mixed model with random intercepts and slopes
...
- to predict vocal frequency from attitude and gender while allowing random intercepts for subject and scenario and random slopes based on attitude
  - Remember to add these to the equation (1+pred|pred2)
...
### Exercise: Check your assumptions for mod.3

- Linearity and homoscedasticity of the model
- Normality of residuals
- Influential data points
  - Try `plot(hatvalues(mod.3))` and see if there is a cut-off you would use to exclude the potentially influential values from the model
  - Figure out a way to find that/those values from the dataset (in a new object) and re-run model without these points



### Check Linearity and homoscedasticity of the model
